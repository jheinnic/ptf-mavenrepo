...
public class StatefulFlowProcessor extends AbstractClassProcessor implements RegisterGlobalsParticipant<ClassDeclaration>, TransformationParticipant<MutableClassDeclaration> {
  ...
  public void doRegisterGlobals(final ClassDeclaration annotatedClass, @Extension final RegisterGlobalsContext context) {
    context.registerClass(this.getManagingSubscriberClass(annotatedClass));
  }
  
  public String getManagingSubscriberClass(final ClassDeclaration annotatedClass) {
    String _qualifiedName = annotatedClass.getQualifiedName();
    return (_qualifiedName + "ManagingSubscriber");
  }
  
  @Override
  public void doTransform(final List<? extends MutableClassDeclaration> elements, @Extension final TransformationContext context) {
    for (final MutableClassDeclaration nextStatefulFlow : elements) {
      {
        final Type managingSubscriber = context.findTypeGlobally(this.getManagingSubscriberClass(nextStatefulFlow));
        this.loadManagingSubscriberMethods(managingSubscriber, nextStatefulFlow, context);
      }
    }
  }
  
  public Iterable<MutableMethodDeclaration> findHandlerMethodDelegates(final MutableClassDeclaration statefulFlow, @Extension final TransformationContext context) {
    Iterable<MutableMethodDeclaration> _xblockexpression = null;
    {
      final MutableAnnotationTypeDeclaration beginFlowAnnotation = context.findAnnotationType("name.jchein.portfolio.common.es.eventuate.statefulflow.BeginFlow");
      final MutableAnnotationTypeDeclaration continueFlowAnnotation = context.findAnnotationType("name.jchein.portfolio.common.es.eventuate.statefulflow.ContinueFlow");
      final MutableAnnotationTypeDeclaration endFlowAnnotation = context.findAnnotationType("name.jchein.portfolio.common.es.eventuate.statefulflow.EndFlow");
      final Function1<MutableMethodDeclaration, List<Element>> _function = (MutableMethodDeclaration nextMethod) -> {
        AnnotationReference _elvis = null;
        AnnotationReference _elvis_1 = null;
        AnnotationReference _findAnnotation = nextMethod.findAnnotation(beginFlowAnnotation);
        if (_findAnnotation != null) {
          _elvis_1 = _findAnnotation;
        } else {
          AnnotationReference _findAnnotation_1 = nextMethod.findAnnotation(continueFlowAnnotation);
          _elvis_1 = _findAnnotation_1;
        }
        if (_elvis_1 != null) {
          _elvis = _elvis_1;
        } else {
          AnnotationReference _findAnnotation_2 = nextMethod.findAnnotation(endFlowAnnotation);
          _elvis = _findAnnotation_2;
        }
        return Collections.<Element>unmodifiableList(CollectionLiterals.<Element>newArrayList(_elvis, nextMethod));
      };
      final Function1<List<Element>, Boolean> _function_1 = (List<Element> nextPair) -> {
        Element _get = nextPair.get(0);
        return Boolean.valueOf((_get != null));
      };
      final Function1<List<Element>, MutableMethodDeclaration> _function_2 = (List<Element> nextPair) -> {
        Element _get = nextPair.get(1);
        return ((MutableMethodDeclaration) _get);
      };
      _xblockexpression = IterableExtensions.<List<Element>, MutableMethodDeclaration>map(IterableExtensions.<List<Element>>filter(IterableExtensions.map(statefulFlow.getDeclaredMethods(), _function), _function_1), _function_2);
    }
    return _xblockexpression;
  }
  
  public void loadManagingSubscriberMethods(final Type managingSubscriber, final MutableClassDeclaration statefulFlow, @Extension final TransformationContext context) {
    final MutableClassDeclaration mutableSubscriber = context.findClass(managingSubscriber.getQualifiedName());
    final Consumer<MutableMethodDeclaration> _function = (MutableMethodDeclaration nextMethod) -> {
      final Procedure1<MutableMethodDeclaration> _function_1 = (MutableMethodDeclaration mutableDeclaration) -> {
        final Function1<MutableParameterDeclaration, Boolean> _function_2 = (MutableParameterDeclaration it) -> {
          return Boolean.valueOf(true);
        };
        mutableDeclaration.addParameter(
          "event", 
          IterableExtensions.findFirst(nextMethod.getParameters(), _function_2).getType());
      };
      mutableSubscriber.addMethod(
        nextMethod.getSimpleName(), _function_1);
    };
    this.findHandlerMethodDelegates(statefulFlow, context).forEach(_function);
  }
